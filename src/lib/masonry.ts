import {flatten, pipe, splitEvery, sort, pick, map, comparator, filter, groupBy, path, transpose, insert, min, takeLast, take, slice, concat, nth, head, tail} from 'ramda'
...
export function chunks(items: any[], chunksCount: number) {
  ...
  function makeChunks(container: any[][], itemsToChunk: any[], index: number, rest: number) {
    ...
    **/
    ...
    return makeChunks(insert(index, take(chunkSize, itemsToChunk), container), takeLast(itemsToChunk.length - chunkSize, itemsToChunk), index + 1, rest - 1)
...
function makeMasonry(columnsCount: number, items: WorkContent[]): WorkContent[][] {
  const itemsPerColumn = Math.floor(items.length / columnsCount)
  ...
  const transposedWorks = flatten(transpose([bigThumbnailsWorks, smallThumbnailWorks]))

  return chunks(transposedWorks, columnsCount)
}
